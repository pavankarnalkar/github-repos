...
describe("SearchBar", () => {
  ...
  it("renders search input with placeholder", () => {
    ...
    const searchInput = screen.getByPlaceholderText(
      TEXT_CONSTANTS.SEARCH.PLACEHOLDER
    );
  ...
  it("renders sort dropdown with correct options", () => {
    ...
    expect(sortSelect).toContainHTML(
      `<option value="stars">${TEXT_CONSTANTS.SEARCH.SORT_OPTIONS.STARS}</option>`
    );
    expect(sortSelect).toContainHTML(
      `<option value="forks">${TEXT_CONSTANTS.SEARCH.SORT_OPTIONS.FORKS}</option>`
    );
    expect(sortSelect).toContainHTML(
      `<option value="updatedAt">${TEXT_CONSTANTS.SEARCH.SORT_OPTIONS.UPDATED}</option>`
    );
    expect(sortSelect).toContainHTML(
      `<option value="createdAt">${TEXT_CONSTANTS.SEARCH.SORT_OPTIONS.CREATED}</option>`
    );
    expect(sortSelect).toContainHTML(
      `<option value="name">${TEXT_CONSTANTS.SEARCH.SORT_OPTIONS.NAME}</option>`
    );
  });
  ...
  it("renders language filter dropdown", () => {
    ...
    expect(languageSelect).toContainHTML(
  ...
  it("displays current sort order correctly", () => {
    ...
    const sortOrderButton = screen.getByTitle(
